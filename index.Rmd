...
::: {.interests}
I am an **Assistant Professor of Statistics** at _Universiti Brunei Darussalam_ (UBD).
...
tab_emp <-
  ...
  select(year, what)
tab_emp$year[3] <- c("2010 --- 2014<br>2018 --- 2019")
kable(tab_emp)
```
...
kable(consultant_tab %>% slice(1))
```

<br class="pageBreak">

```{r}
kable(consultant_tab %>% slice(2))
```
...
knitr::kable(tab %>% slice(1:5))
```

<br class="pageBreak">

```{r}
knitr::kable(slice(tab, -(1:5)))
```
...
knitr::kable(tab_wp %>% slice(1:4))
```

<br class="pageBreak">

```{r}
knitr::kable(slice(tab_wp, -(1:4)))
```
...
knitr::kable(talk_tab %>% slice(1:2))
```
