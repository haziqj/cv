...
::: {.contact-info}
haziq.jamil@kaust.edu.sa <i class="fas fa-fw fa-envelope"></i>
...
::: {.interests}
I am a Research Specialist working at King Abdullah University of Science and Technology (KAUST), currently on leave from my position as an Assistant Professor in Statistics at Universiti Brunei Darussalam (UBD).
...
tab_emp <-
  ...
  mutate(additional_info = gsub(
    "[Leave of absence from August 2025]<br>", "", additional_info, fixed = TRUE
  )) %>%
  mutate(what = glue("<span class='cv-entry-title'>{what}</span><br>",
                     "{where}<br>", "<span class='cv-entry-gray'>{additional_info}</span>")) %>% 
  select(year, what) 
tab_emp <- tab_emp[c(1, 3), ]  # only UBD and KAUST
tab_emp$year[2] <- c("2019 --- 2025")
# tab_emp$what[2] <-
#   gsub(
#   "\\<br\\>\\<span class='cv-entry-gray'\\>\\[Leave of absence from August 2025\\]", "",
#   tab_emp$what[2]
# )
kable(tab_emp)
```
...
tab_sw$venue[c(5, 4, 3)] <- ""
tab_sw$venue[2] <- "<img src=\"https://img.shields.io/github/downloads/haziqj/stat-tables/total.svg?color=yellow\">"
tab_sw <-
  tab_sw %>%
  mutate(citation = glue("{citation}",
                         "<span class='publication-authors'>{authors_full}</span><br>",
